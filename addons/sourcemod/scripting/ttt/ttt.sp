...
public Action Event_PlayerDeathPre(Event event, const char[] menu, bool dontBroadcast) {
    ...
    if (g_iPlayer[client].Role > TTT_TEAM_UNASSIGNED) {
        ...
        if (g_iStatus == Round_Active && (client != iAttacker && iAttacker != 0 && !g_iPlayer[iAttacker].ImmuneRDMManager)) {
            if (
              (g_iPlayer[iAttacker].Role == TTT_TEAM_TRAITOR && g_iPlayer[client].Role == TTT_TEAM_TRAITOR) || (g_iPlayer[iAttacker].Role == TTT_TEAM_DETECTIVE && g_iPlayer[client].Role == TTT_TEAM_DETECTIVE) || (g_cShowInnoRDMMenu.BoolValue && g_iPlayer[iAttacker].Role == TTT_TEAM_INNOCENT && g_iPlayer[client].Role == TTT_TEAM_INNOCENT)) {
...
public Action Event_PlayerHurt(Event event, const char[] name, bool dontBroadcast) {
    ...
    if ((g_iPlayer[iAttacker].Role == TTT_TEAM_INNOCENT && g_iPlayer[client].Role == TTT_TEAM_INNOCENT) || (g_iPlayer[iAttacker].Role == TTT_TEAM_INNOCENT && g_iPlayer[client].Role == TTT_TEAM_DETECTIVE) || (g_iPlayer[iAttacker].Role == TTT_TEAM_TRAITOR && g_iPlayer[client].Role == TTT_TEAM_TRAITOR) || (g_iPlayer[iAttacker].Role == TTT_TEAM_DETECTIVE && g_iPlayer[client].Role == TTT_TEAM_INNOCENT) || (g_iPlayer[iAttacker].Role == TTT_TEAM_DETECTIVE && g_iPlayer[client].Role == TTT_TEAM_DETECTIVE)) {
...
public Action Timer_5(Handle timer) {
    ...
    if (g_iStatus == Round_Active) {
        CheckTeams();
    } else if (g_bCheckPlayers || (g_cCheckDuringWarmup.BoolValue && (TTT_IsWarmUp() || g_iStatus <= Round_Warmup)) || (g_cCheckPlayers.BoolValue && (!TTT_IsWarmUp() && g_iStatus <= Round_Warmup && g_hStartTimer == null && g_hCountdownTimer == null && g_hRoundTimer == null))) {
        CheckPlayers();
    }
...
int DetermineTeam() {
    ...
    return tCount == ctCount ? GetRandomInt(CS_TEAM_T, CS_TEAM_CT) : tCount < ctCount ? CS_TEAM_T
                                                                                      : CS_TEAM_CT;
}
